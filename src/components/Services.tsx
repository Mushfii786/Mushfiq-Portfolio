...
export function Services() {
  const getServiceIcon = (iconName: string) => {
    switch (iconName) {
      case 'Layout': return <Layout className="w-6 h-6 text-[#FF4D12]" />;
      case 'Code': return <Code className="w-6 h-6 text-[#FF4D12]" />;
      case 'Palette': return <Palette className="w-6 h-6 text-[#FF4D12]" />;
      case 'Camera': return <Camera className="w-6 h-6 text-[#FF4D12]" />;
      default: return <Sparkles className="w-6 h-6 text-[#FF4D12]" />;
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {services.map((service, index) => (
          <motion.div
            ...
            whileHover={{ y: -6 }}
            className="group relative p-8 rounded-[2rem] bg-white dark:bg-neutral-900/70 border border-neutral-200 dark:border-neutral-800/80 shadow-lg shadow-neutral-200/50 dark:shadow-black/20 flex flex-col justify-between hover:border-[#FF4D12] transition-all duration-300"
          >
            <div>
              <div className="flex justify-between items-start mb-6">
                <div className="p-3.5 rounded-2xl bg-[#FF4D12]/10 border border-[#FF4D12]/20 group-hover:scale-110 transition-transform">
              ...
              <p className="text-sm font-semibold text-[#FF4D12] mb-4">
                "{service.tagline}"
              </p>
              ...
              <div className="space-y-2 pt-4 border-t border-neutral-200 dark:border-neutral-800/60">
                ...
                {service.deliverables.map((item, dIdx) => (
                  <div key={dIdx} className="flex items-center gap-2 text-xs font-medium text-neutral-800 dark:text-neutral-300">
                    <Check className="w-3.5 h-3.5 text-[#FF4D12]" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-8 pt-4">
              <a
                href="#contact"
                className="inline-flex items-center gap-2 text-xs font-mono font-semibold text-neutral-900 dark:text-white group-hover:text-[#FF4D12] transition-colors"
                data-cursor-text="Inquire"
              >
                <span>Request Proposal</span>
                <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform text-[#FF4D12]" />
              </a>
            </div>
          </motion.div>
        ))}
